...
/**
 * Controller responsável por validar e orquestrar movimentos de stock.
 *
 * Aplica as regras de negócio (compras, vendas, ajustes, transferências) e
 * garante consistência via transações (HikariCP).
 *
 * @author Hulquene
 */
public class StockMovementController {
    ...
    // ==========================================================
    // 🔹 Registro com validações e transações
    // ==========================================================
    /**
     * Registra um movimento de stock aplicando regras de negócio. - Verifica
     * stock suficiente para SAÍDA/TRANSFERÊNCIA - Impede novas ENTRADAS se a
     * compra estiver completa - Usa transações para garantir consistência
     */
    public boolean registrar(StockMovement movimento) {
        try {
            ...
            // 🔐 Transação opcional — apenas se o movimento afetar mais de uma tabela
            try (Connection conn = DatabaseProvider.getConnection()) {
                conn.setAutoCommit(false);

                StockMovementDao daoTx = new StockMovementDao(conn);
                PurchaseItemDao purchaseDaoTx = new PurchaseItemDao(conn);

                // Revalida se necessário
                if ("COMPRA".equalsIgnoreCase(movimento.getOrigin())) {
                    if (!purchaseDaoTx.podeDarEntradaDeCompra(movimento)) {
                ...
                // ✅ Executa inserção
                boolean ok = daoTx.add(movimento);
    ...
    private boolean validarEntrada(StockMovement movimento, String origem) {
        ...
        // Outros tipos de entrada (devolução, transferência, ajuste) não têm restrição
        return true;
    }
    ...
    private boolean validarSaida(StockMovement movimento, String origem) {
        ...
        // Saídas são sempre gravadas como negativas
